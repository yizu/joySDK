...
var Utils = {
  ...
  getIdealType: function( $el ) {
    ...
    return (
      /(text|password|password2|email|number|search|url|tel|textarea)/.test( type ) && 'text' ||
      /file/.test( type ) && 'file' ||
  ...
  makeInput: function( name, value, type, list, placeholder ) {
    ...
    // Text & file
    if ( /^(text|password|password2|email|number|search|url|tel|file|hidden)$/.test(type) )
      markup = '<input '+
        'type="'+ type +'" '+
        'id="'+ name +'" '+
        'name="'+ name +'" '+
...
var _defaults = {
  ...
  customFlags: {},
  globalFlags: '',
  onSuccess: function(e) {alert('提交成功')},
  onFail: function() { alert('提交失败') },
  responsiveAt: 'auto',
  disableCustom: ''
}
